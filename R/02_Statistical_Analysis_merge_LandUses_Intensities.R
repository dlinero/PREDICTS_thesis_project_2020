...
Merge_landUses_and_intensities <- function(dataset, 
  ...
  name_predominant_habitat_column <- paste("Predominant_land_use", index, sep = ".")
  name_use_intensity_column <- paste("Use_intensity", index , sep = ".")
  name_LandUse <- paste("LandUse", index, sep = ".")
 ...
 dataset %>%
  ...
  mutate(
    ...
    !!name_predominant_habitat_column := paste(Predominant_land_use),
    
    # collapse primary forest and non-forest together into primary vegetation as these aren't well distinguished
    !!name_predominant_habitat_column := recode_factor(!!as.name(name_predominant_habitat_column), 
                                           "Primary forest" = "Primary", 
                                           "Primary non-forest" = "Primary"),
    
    # indeterminate secondary veg and cannot decide are transformed into NA, urban too 
    !!name_predominant_habitat_column := na_if(!!as.name(name_predominant_habitat_column), "Secondary vegetation (indeterminate age)"),
    !!name_predominant_habitat_column := na_if(!!as.name(name_predominant_habitat_column), "Cannot decide"),
    !!name_predominant_habitat_column := na_if(!!as.name(name_predominant_habitat_column), "Urban"),
    
    
    # Give a shorter name to some land use categories 
    !!name_predominant_habitat_column := str_replace_all(!!as.name(name_predominant_habitat_column), pattern = c("Young secondary vegetation" = "YSV",
    ...
    # comes from the previous step where we paste the original name and the index
    !!name_use_intensity_column := paste(Use_intensity),
   ...
   !!name_use_intensity_column := ifelse((!!as.name(name_predominant_habitat_column) %in% land_uses_merge_light_intense) & 
                             ...
                             str_replace_all(!!as.name(name_use_intensity_column),
    ...
    !!name_use_intensity_column := ifelse(!!as.name(name_predominant_habitat_column) %nin% land_uses_separate_intensities,
                             str_replace_all(!!as.name(name_use_intensity_column), pattern = c("Intense use" = "All", 
                                                                          "Light use" = "All", 
                                                                          "Minimal use" = "All", 
                                                                          "Cannot decide" = "All")), 
                             paste(!!as.name(name_use_intensity_column))),
    ...
    !!name_LandUse := ifelse(!!as.name(name_predominant_habitat_column) != "NA" & !!as.name(name_use_intensity_column) != "NA",
                     paste(!!as.name(name_predominant_habitat_column), !!as.name(name_use_intensity_column)),
                     NA),
    
    # set reference level
    !!name_LandUse := factor(!!as.name(name_LandUse), ordered = FALSE),
    !!name_LandUse := relevel(!!as.name(name_LandUse), ref = "Primary Minimal use")
  )
}
