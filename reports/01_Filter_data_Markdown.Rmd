...
```{r Take a look at the table, results='asis'}
kable(Plantspecies[c(1,141,730), c(3,5,8)], "pandoc", caption = "Fragment of the resulting table")
```
...
kable(Updated_plant_names[c(1, 401, 5076,5077), c(2,4,5,6)], "pandoc", caption = "Fragment of the resulting table showing names that are already update (score > 0.988), and a species that has a different nomeclature in the Catalague of life (score = 0.75) including a synonym ")
...
I will start selecting plants with endozoochorous seed dispersal from the Dispersal syndrome table. Therefore, I am going to eliminate all of the records that explicitly state a different seed dispersal syndrome. Additionally, I will also eliminate the records where not mammal or bird were included as consumers
...
# Records per land-use and intensity level
table(PREDICTSendooPlants$Predominant_habitat, PREDICTSendooPlants$Use_intensity)
