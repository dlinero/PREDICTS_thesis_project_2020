...
# Check number of sites
table(diversity_all_abundance$LandUse.0)
...
species <- readRDS(file = "./output/intermediate_files/02_Statistical_Analysis_Abundance_species_all.rds")

# Extract the sites that we use in the model
# Create a character vector of unique SSBS that we use in the model
list <- as.character(unique(diversity_all_abundance$SSBS))
...
pdf(file = "./output/graphs_tables/02_Statistical_Analysis_Abundance_model_results.pdf", width = 10)
